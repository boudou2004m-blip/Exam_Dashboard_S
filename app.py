        ...
        if st.button("Generate Exam Schedule", type="primary", use_container_width=True):
                ...
                if not slots:
                       ...
                       with st.spinner("Generating schedule..."):
                                        ...
                                        for slot in slots:
                                                ...
                                                for gi, group in enumerate(groups, 1):
                                                        ...
                                                        for s in group:
                                                                ...
                                                                exams_today = [
                                                                        d for d in student_exams.get(s["id"], set())
                                                                        if d.date() == exam_date
                                                                ]
                                                        ...
                                                        for p in sorted_profs:
                                                                ...
                                                                exams_today = [
                                                                        d for d in professor_exams.get(p["id"], set())
                                                                        if d.date() == exam_date
                                                                ]
                                                                if len(exams_today) >= 3:
                                                                        continue
                                ...
                                st.session_state.failed_modules = failed_modules


        # ---------- DISPLAY RESULT ----------
        if st.session_state.generated_schedule:

                df = pd.DataFrame(st.session_state.generated_schedule)
                st.dataframe(
                        df.drop(columns=["module_id", "prof_id", "room_id", "date_time"]),
                        use_container_width=True
                )

                if st.session_state.failed_modules:
                        st.warning(
                                f"Could not schedule {len(st.session_state.failed_modules)} module(s): "
